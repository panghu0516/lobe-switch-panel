...
#### 📏 CPU/内存单位写法（K8s quantity）
模式数值必须用 Kubernetes 标准 quantity 写法，建议直接用规范形式（K8s 读写回会规范化，校验按数值比较已容忍差异，但规范写法最直观）：

| 用途 | 推荐写法 | 含义 |
|------|----------|------|
| CPU | `500m` / `1` | 0.5 核 / 1 核（`m` = 毫核；不建议写 `0.5`、`1000m`，会被规范化成 `500m`/`1`） |
| 内存（二进制） | `512Mi` / `1Gi` / `2Gi` | MiB / GiB（不建议写 `0.5Gi`，会被规范化成 `512Mi`；`1024Mi`→`1Gi`） |
| 内存（十进制） | `500M` / `1G` | 按 1000 进制；写 `1G` 读回是 `1000000000`，和 `1Gi` 不相等，注意别混用 |
